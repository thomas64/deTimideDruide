...
class PouchItemDatabase(enum.Enum):
    ...
    grapes = dict(nam="Druiven",                    srt=2, spr=GRAPESPATH,          desc="Ingredient voor een goede wijn", val=1)
    wine = dict(nam="Wijn",                         srt=3, spr=WINEPATH,            desc="Een heerlijke wijn. Predicaat ok.", val=5)
    bottle = dict(nam="Lege fles",                  srt=4, spr=BOTTLEPATH,          desc="Een lege fles. Handig voor het bewaren van vloeistoffen")
    cauldron = dict(nam="Magische Ketel",           srt=5, spr=CAULDRONPATH,        desc="Ketel met magische eigenschappen die Zwammix helpen bij het bereiden van zijn soep")
    recept1 = dict(nam="Recept blz 1",              srt=6, spr=RECEPT,              desc="Een bladzijde uit het kookboek van Zwammix", click=IMGPATH+'recept1.png')
    recept2 = dict(nam="Recept blz 2",              srt=6, spr=RECEPT,              desc="Een bladzijde uit het kookboek van Zwammix", click=IMGPATH+'recept2.png')
    recept3 = dict(nam="Recept blz 3",              srt=6, spr=RECEPT,              desc="Een bladzijde uit het kookboek van Zwammix", click=IMGPATH+'recept3.png')

    droppings = dict(nam="Lepus stercora",          srt=9, spr=DROPPINGSPATH,       desc="Ingredient van de Zwammix")
    shroom = dict(nam="Agaricus bisporus",          srt=10, spr=SHROOMSPATH,        desc="Ingredient van de Zwammix")
    rose = dict(nam="Surrexit petalis",             srt=11, spr=ROSEPATH,           desc="Ingredient van de Zwammix")
    ladybug = dict(nam="Coccinellidae",             srt=12, spr=LADYBUGPATH,        desc="Ingredient van de Zwammix")
    wool = dict(nam="Oves Lana",                    srt=13, spr=WOOLPATH,           desc="Ingredient van de Zwammix")
    shells = dict(nam="Conchis",                    srt=14, spr=SHELLSPATH,         desc="Ingredient van de Zwammix")
    poppy = dict(nam="Papaver somniferum",          srt=15, spr=POPPYPATH,          desc="Ingredient van de Zwammix")
    cantharel = dict(nam="Cantharellus cibarius",   srt=16, spr=CANTHARELPATH,      desc="Ingredient van de Zwammix")
    weegbree = dict(nam="Plantago",                 srt=17, spr=WEEGBREEPATH,       desc="Ingredient van de Zwammix")
    berkenschors = dict(nam="Betulis cortice",      srt=18, spr=BERKENSCHORSPATH,   desc="Ingredient van de Zwammix")
    maretak = dict(nam="Visci",                     srt=19, spr=MARETAKPATH,        desc="Ingredient van de Zwammix")
    spiderweb = dict(nam="Araneo",                  srt=20, spr=SPIDERWEBPATH,      desc="Ingredient van de Zwammix")
    fluitekruid = dict(nam="Fluitekruid",           srt=21, spr=FLUITEKRUIDPATH,    desc="Ingredient van de Zwammix")
    knoflook = dict(nam="Knoflook",                 srt=22, spr=KNOFLOOKPATH,       desc="Ingredient van de Zwammix")
    shallot = dict(nam="Shallot",                   srt=23, spr=SHALLOTPATH,        desc="Ingredient van de Zwammix")
    tomaat = dict(nam="Tomaat",                     srt=24, spr=TOMAATPATH,         desc="Ingredient van de Zwammix")
    doperwten = dict(nam="Doperwten",               srt=25, spr=DOPERWTENPATH,      desc="Ingredient van de Zwammix")

    orakelsteen = dict(nam="Orakelsteen",           srt=26, spr=ORAKELSTEEN,        desc="Steen met bijzondere voorspellende eigenschappen")
    putsi = dict(nam="Putsi",                       srt=27, spr=PUTSI,              desc="Beertje met hoog knuffelgehalte")
    niers = dict(nam="Kaart Niers",                 srt=27, spr=LANDKAART,          desc="Kaart van de rivier de Niers", click=IMGPATH+'niers.png')
    water_waal = dict(nam="Water Waal",             srt=28, spr=WATERRIVIER,        desc="Ingredient voor drankje", val=3)
    water_maas = dict(nam="Water Maas",             srt=28, spr=WATERRIVIER,        desc="Ingredient voor drankje", val=3)
    water_niers = dict(nam="Water Niers",           srt=28, spr=WATERRIVIER,        desc="Ingredient voor drankje", val=3)
    bloem_geel = dict(nam="Gele bloem",             srt=29, spr=BLOEMGEEL,          desc="Ingredient voor drankje", val=3)
    bloem_blauw = dict(nam="Blauwe bloem",          srt=29, spr=BLOEMBLAUW,         desc="Ingredient voor drankje", val=3)
    bloem_rood = dict(nam="Rode bloem",             srt=29, spr=BLOEMROOD,          desc="Ingredient voor drankje", val=3)
